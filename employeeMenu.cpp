...
void readFromFile(); // option 9
void readFromFileToArr(Employee_record [], int);
...
int main()
{
    ...
    //readFromFileToArr(employee_arr, NUM_RECORDS);
/*
...
void readFromFileToArr(Employee_record arr[], int size) {
    fstream employeeFile("employee.txt", ios::in);
    int i = 0;
    if (employeeFile.is_open()) {
        while (!employeeFile.eof()) {
            employeeFile
                >> arr[i].idNumber >> arr[i].employeeName
                >> arr[i].birthDate.month >> arr[i].birthDate.day >> arr[i].birthDate.year
                >> arr[i].employeeAddress1 >> arr[i].employeeAddress2
                >> arr[i].mobileNumber >> arr[i].emailID >> arr[i].department
                >> arr[i].hireDate.month >> arr[i].hireDate.day >> arr[i].hireDate.year
                >> arr[i].employeeSalary;
            i++;
        }
        cout << "Finished reading file contents to the array.\n";
        employeeFile.close();
    }
    else {
        cout << "Error opening file.\n";
